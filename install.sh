...
check_docker() {
  ...
  if ! [[ "$(printf '%s\n' "$min_version" "$version" | sort -V | head -n1)" = "$min_version" ]]; then
    log "Docker version $version is installed, but BoltR requires version $min_version or higher." $COLOR_RED
...
check_docker_compose() {
  ...
  if ! [[ "$(printf '%s\n' "$min_version" "$version" | sort -V | head -n1)" = "$min_version" ]]; then
    log "Docker Compose version $version is installed, but BoltR requires version $min_version or higher." $COLOR_RED
...
remove_old_images() {
  log "Checking for old BoltR/reNgine Docker containers and images..." $COLOR_CYAN
  
  # Stop and remove all containers
  if docker ps -a --format '{{.Names}}' | grep -qE '^rengine-|^postgres$|^redis$'; then
    log "Stopping existing BoltR containers..." $COLOR_YELLOW
    if ! make down; then
...
main() {
  ...
  log "Checking and installing BoltR prerequisites..." $COLOR_CYAN
  ...
  if [ "$current_id" -ne 1000 ]; then
      ...
      INSTALL_TYPE="source"
      log "Build has been forced because your user ID is not the same as the pre-built images. If you want to use pre-built images, your current user installing BoltR must be 1000." $COLOR_RED
  else
      if [ "$isNonInteractive" = false ]; then
  ...
  log "Installing BoltR from $INSTALL_TYPE, please be patient as the installation could take a while..." $COLOR_CYAN
  sleep 5

  log "Generating certificates..." $COLOR_CYAN
  make certs && { log "Certificates have been generated" $COLOR_GREEN; true; } || { log "Certificate generation failed!" $COLOR_RED; exit 1; }

  if [ "$INSTALL_TYPE" = "source" ]; then
    log "Building Docker images..." $COLOR_CYAN
    make build && { log "Docker images have been built" $COLOR_GREEN; true; } || { log "Docker images build failed!" $COLOR_RED; exit 1; }
  fi

  if [ "$INSTALL_TYPE" = "prebuilt" ]; then
    log "Pulling pre-built Docker images..." $COLOR_CYAN
    make pull && { log "Docker images have been pulled" $COLOR_GREEN; true; } || { log "Docker images pull failed!" $COLOR_RED; exit 1; }
  fi

  log "Docker containers starting, please wait as starting the Celery container could take a while..." $COLOR_CYAN
  sleep 5
  make up && { log "BoltR is started!" $COLOR_GREEN; true; } || { log "BoltR start failed!" $COLOR_RED; exit 1; }
  ...
  log "BoltR is successfully installed and started!" $COLOR_GREEN
  log "\r\nThank you for installing BoltR, happy recon!" $COLOR_GREEN

  log "\r\nIn case you're running this locally, BoltR should be available at one of the following IPs:\n$formatted_ips" $COLOR_GREEN
  log "In case you're running this on a server, BoltR should be available at: https://$external_ip/" $COLOR_GREEN
}
