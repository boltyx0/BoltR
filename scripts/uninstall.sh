...
if [ "$(whoami)" != "root" ]
  then
  log ""
  log "Error uninstalling BoltR: please run this script as root!" $COLOR_RED
  log "Example: sudo ./uninstall.sh" $COLOR_RED
  exit
fi

log ""
log "Uninstalling BoltR..." $COLOR_CYAN
log ""

tput setaf $COLOR_RED;
read -p "This action will stop and remove all containers, volumes and networks of BoltR. Do you want to continue? [y/n] " -n 1
log ""
...
then
  ...
  log "Stopping BoltR..." $COLOR_CYAN
  if (cd .. && make down); then
    log "Stopped BoltR" $COLOR_GREEN
  else
    log "Failed to stop BoltR" $COLOR_RED
    exit 1
  fi
  log ""

  log "Removing all volumes related to BoltR..." $COLOR_CYAN
  if docker volume rm $(docker volume ls -q --filter name=rengine_) 2>/dev/null || true; then
    log "Removed all volumes related to BoltR" $COLOR_GREEN
  else
    log "Warning: Failed to remove some or all volumes" $COLOR_YELLOW
  fi
  log ""

  log "Removing all networks related to BoltR..." $COLOR_CYAN
  if docker network rm rengine_network; then
    log "Removed all networks related to BoltR" $COLOR_GREEN
  else
    log "Warning: Failed to remove rengine_network" $COLOR_YELLOW
  fi
  log ""

  log "Removing static files and secrets from BoltR..." $COLOR_CYAN
...
tput setaf $COLOR_RED;
read -p "Do you want to remove Docker images related to BoltR? [y/n] " -n 1 -r
log ""

if [[ $REPLY =~ ^[Yy]$ ]]
then
  log ""
  log "Removing all Docker images related to BoltR..." $COLOR_CYAN
