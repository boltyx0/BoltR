...
def detail_scan(request, id, slug):
    ...
    # Tool Arsenal tab: show if this scan has a Tool Arsenal activity
    has_tool_arsenal_activity = scan_activity.filter(name='run_tool_arsenal_scan').exists()
    ...
    ctx = {
        ...
        'has_tool_arsenal_activity': has_tool_arsenal_activity,
...
def stop_scan(request, slug, id):
    if request.method == "POST":
        ...
        scan.aborted_by = request.user if request.user.is_authenticated else None
        scan.save()
        try:
        ...
        # Decide response type: JSON for AJAX/JS calls, redirect for normal form POST
        is_json = request.headers.get('x-requested-with') == 'XMLHttpRequest' or request.content_type == 'application/json'
        if is_json:
            return JsonResponse(response)
        # For standard form submissions, go back to scan history page
        return HttpResponseRedirect(reverse('scan_history', kwargs={'slug': slug}))
