...
function get_logs_modal(scan_id=null, activity_id=null) {
	...
	.then(data => {
		swal.close();
		if (scan_id) {
			$('#xl-modal-title').html(`Logs for scan #${scan_id}`);
		} else {
			$('#xl-modal-title').html(`Activity logs${typeof scan_history_id !== 'undefined' ? ' (scan #' + scan_history_id + ')' : ''}`);
		}
...
// Tool Arsenal tab: load, filter, and render output
function escapeHtmlToolArsenal(text) {
...
// Strip ANSI escape codes so output is readable in the UI
function stripAnsiToolArsenal(text) {
	if (text == null || text === '') return '';
	return String(text).replace(/\x1b\[[0-9;]*[mGKHfJ]/g, '').replace(/\x1b\[\?[0-9;]*[hl]/g, '');
}

// Keep only result-like lines (findings, parameters, vulnerabilities) and drop progress noise
function summarizeToolOutput(text) {
	if (text == null || text === '') return '';
	var cleaned = stripAnsiToolArsenal(text);
	var lines = cleaned.split(/\r?\n/);
	var resultKeywords = /found|parameter|vulnerable|detected|injection|secret|key:|match|finding|issue|critical|high|medium|low|\[+\]|\[!\]|\[-\]|^\s*[a-zA-Z0-9_.-]+:\s*\S/i;
	var noisePatterns = /^\[?\s*(\d+%?|\d+\/\d+)\s*\]?$|processing chunks|scanning\.\.\.|please wait|loading\.\.\.|^[\s\-|]*$/i;
	var kept = [];
	lines.forEach(function (line) {
		var t = line.trim();
		if (!t) return;
		if (noisePatterns.test(t)) return;
		if (resultKeywords.test(t) || t.length > 60) kept.push(line);
	});
	if (kept.length > 0) return kept.join('\n');
	// If nothing matched, return last 30 non-empty lines to avoid hiding everything
	var nonEmpty = lines.filter(function (l) { return l.trim().length > 0; });
	var tail = nonEmpty.slice(-30);
	return tail.length > 0 ? tail.join('\n') : cleaned;
}

function renderToolArsenalOutput(results, toolFilter, searchFilter) {
	var container = document.getElementById('tool-arsenal-results');
	var emptyEl = document.getElementById('tool-arsenal-empty');
	var showFull = (document.getElementById('tool-arsenal-show-full') || {}).checked;
	if (!container) return;
	var filtered = (results || []).filter(function (r) {
		if (toolFilter && r.tool_name !== toolFilter) return false;
		if (searchFilter) {
			var s = (searchFilter || '').toLowerCase();
			var cmd = (r.command || '').toLowerCase();
			var out = (r.output || '').toLowerCase();
			if (cmd.indexOf(s) === -1 && out.indexOf(s) === -1) return false;
		}
		return true;
	});
	if (filtered.length === 0) {
		...
		emptyEl.textContent = toolFilter || searchFilter ? 'No matching output.' : 'No Tool Arsenal output for this scan yet.';
		return;
	}
	...
	filtered.forEach(function (r) {
		var rawOut = (r.output || '') + (r.error_output ? '\n' + r.error_output : '');
		var displayOut = showFull ? stripAnsiToolArsenal(rawOut) : summarizeToolOutput(rawOut);
		var cmdEsc = escapeHtmlToolArsenal(r.command);
		var outEsc = escapeHtmlToolArsenal(displayOut);
		var badgeClass = 'badge-soft-primary';
		html += '<div class="card mb-2"><div class="card-body">';
		html += '<span class="badge ' + badgeClass + ' mb-2">' + escapeHtmlToolArsenal(r.tool_name) + '</span>';
		html += '<p class="mb-1"><small class="text-muted">Command:</small><br><code class="d-block p-2 bg-light" style="word-break: break-all;">' + cmdEsc + '</code></p>';
		html += '<p class="mb-0"><small class="text-muted">Output:</small></p>';
		html += '<pre class="p-2 bg-dark text-light rounded" style="max-height: 400px; overflow: auto; font-size: 12px;">' + (displayOut ? outEsc : '(No result lines captured)') + '</pre>';
		html += '</div></div>';
	});
	container.innerHTML = html;
}

function loadToolArsenalOutput(scanId) {
	var loading = document.getElementById('tool-arsenal-loading');
	var resultsEl = document.getElementById('tool-arsenal-results');
	var emptyEl = document.getElementById('tool-arsenal-empty');
	if (!loading || !resultsEl) return;
	loading.style.display = 'block';
	resultsEl.style.display = 'none';
	if (emptyEl) emptyEl.style.display = 'none';
	fetch('/api/queryScanToolArsenalOutput/?scan_id=' + encodeURIComponent(scanId) + '&format=json')
		.then(function (res) { return res.json(); })
		.then(function (data) {
			loading.style.display = 'none';
			var toolFilter = (document.getElementById('tool-arsenal-filter-tool') || {}).value || '';
			var searchFilter = (document.getElementById('tool-arsenal-filter-search') || {}).value || '';
			window._toolArsenalData = data.results || [];
			renderToolArsenalOutput(window._toolArsenalData, toolFilter, searchFilter);
		})
		.catch(function () {
			loading.style.display = 'none';
			if (emptyEl) { emptyEl.style.display = 'block'; emptyEl.textContent = 'Failed to load Tool Arsenal output.'; }
		});
}

function initToolArsenalTab() {
	var tab = document.getElementById('tool-arsenal-tab');
	if (!tab) return;
	var scanId = tab.getAttribute('data-scan-id');
	if (!scanId) return;
	var shown = false;
	$('#tool-arsenal-tab').on('shown.bs.tab', function () {
		if (!shown) { loadToolArsenalOutput(scanId); shown = true; }
	});
	$(document).on('change', '#tool-arsenal-filter-tool', function () {
		if (window._toolArsenalData) renderToolArsenalOutput(window._toolArsenalData, this.value, (document.getElementById('tool-arsenal-filter-search') || {}).value);
	});
	$(document).on('input', '#tool-arsenal-filter-search', function () {
		if (window._toolArsenalData) renderToolArsenalOutput(window._toolArsenalData, (document.getElementById('tool-arsenal-filter-tool') || {}).value, this.value);
	});
	$(document).on('click', '#tool-arsenal-refresh-btn', function () {
		loadToolArsenalOutput(scanId);
	});
	$(document).on('change', '#tool-arsenal-show-full', function () {
		if (window._toolArsenalData) renderToolArsenalOutput(window._toolArsenalData, (document.getElementById('tool-arsenal-filter-tool') || {}).value, (document.getElementById('tool-arsenal-filter-search') || {}).value);
	});
}
