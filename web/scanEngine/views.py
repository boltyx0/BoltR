...
def tool_specific_settings(request):
    ...
    if request.method == "POST":
        handled_key = handle_post_request(request)
        redirect_url = reverse('tool_settings')
        if handled_key == 'nucleiFileUpload':
            redirect_url += '#nuclei-template'
        return http.HttpResponseRedirect(redirect_url)

    nuclei_glob = sorted(glob.glob(str(Path.home() / "nuclei-templates" / "*.yaml")))
    context = {
        'settings_nav_active': 'active',
        'tool_settings_li': 'active',
        'settings_ul_show': 'show',
        'gf_patterns': get_gf_patterns(request),
        'nuclei_templates': [os.path.basename(p) for p in nuclei_glob]
    }
...
def handle_post_request(request):
    ...
    for key, handler in handlers.items():
        if key in request.FILES or key in request.POST:
            # For file uploads, only run if a file was actually selected
            if key in request.FILES and request.FILES[key].name:
                handler(request)
                return key
            if key in request.POST and key not in request.FILES:
                handler(request)
                return key
    return None
