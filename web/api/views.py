    ...
    def get(self, request):
        try:
            ...
            if not tool_id and not tool_name:
                return Response({'status': False, 'message': 'tool_id or name is required.'}, status=HTTP_400_BAD_REQUEST)
            ...
            try:
                if tool_id:
                    ...
                    tool = InstalledExternalTool.objects.get(name=tool_name)
            except InstalledExternalTool.DoesNotExist:
                return Response({'status': False, 'message': 'Tool not found.'}, status=HTTP_400_BAD_REQUEST)
            ...
            update_command = (tool.update_command or '').strip().lower()
            ...
            if not update_command:
                ...
                _raw = (tool.install_command or '').strip().rstrip('/')
                _tool_dir = (_raw.split('/')[-1] or '').removesuffix('.git')
                if not _tool_dir:
                    return Response({'status': False, 'message': tool.name + ': could not determine git repo directory from install command.'}, status=HTTP_400_BAD_REQUEST)
                update_command = 'cd ' + str(Path(RENGINE_TOOL_GITHUB_PATH) / _tool_dir) + ' && git pull && cd -'

            # Queue update to run in Celery worker (tools run inside celery container).
            # shell=True required for "cd ... &&" and multi-part commands.
            run_command.apply_async(args=(update_command,), kwargs={'shell': True})
            return Response({'status': True, 'message': tool.name + ' update started. It runs in the background and may take a few minutes.'})
        except Exception as e:
            return Response({'status': False, 'message': 'Server error: ' + str(e)}, status=500)
...
class GetExternalToolCurrentVersion(APIView):
    def get(self, request):
        ...
        # For github-cloned tools, run from clone path via cwd so we don't need shell.
        version_cmd = tool.version_lookup_command.strip()
        cwd = None
        if tool.github_clone_path:
            cwd = tool.github_clone_path
            # If command is "cd /path && ...", use only the part after " && " and rely on cwd.
            if version_cmd.startswith('cd ') and ' && ' in version_cmd:
                version_cmd = version_cmd.split(' && ', 1)[1].strip()
        version_number = None
        try:
            task_result = run_command.delay(
                version_cmd,
                cwd=cwd,
                combine_output=True
            )
            ...
            # Debug logs
            logger.debug(f"Command: {version_cmd}, cwd: {cwd}")
            ...
            err_msg = str(e)
            if cwd and ('No such file or directory' in err_msg or 'Errno 2' in err_msg):
                return Response({'status': False, 'message': 'Tool not installed (clone path does not exist). Install the tool first.'})
            return Response({'status': False, 'message': f'Error executing version command: {err_msg}'})
...
def _infer_tool_name_from_command(cmd):
    """Infer Tool Arsenal tool name from command text for filtering."""
    if not cmd:
        return 'Other'
...
class GetScanToolArsenalOutput(APIView):
    """Return Tool Arsenal command outputs for a scan for the dedicated Tool Arsenal tab."""
    def get(self, request, format=None):
        scan_id = safe_int_cast(request.query_params.get('scan_id'))
        if not scan_id:
            return Response({'activity_id': None, 'results': []}, status=HTTP_400_BAD_REQUEST)
        activity = ScanActivity.objects.filter(
            scan_of_id=scan_id,
            name='run_tool_arsenal_scan'
        ).first()
        if not activity:
            return Response({'activity_id': None, 'results': []})
        commands = Command.objects.filter(activity_id=activity.id).order_by('id')
        results = []
        for cmd in commands:
            results.append({
                'id': cmd.id,
                ...
                'error_output': getattr(cmd, 'error_output', None) or '',
                'return_code': cmd.return_code,
                'time': cmd.time.isoformat() if cmd.time else None,
                'tool_name': _infer_tool_name_from_command(cmd.command),
            })
        return Response({'activity_id': activity.id, 'results': results})
