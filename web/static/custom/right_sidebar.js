    ...
    if (scans['scanning'].length > 0){
      ...
      for (var scan in scans['scanning']) {
        ...
        $('#currently_scanning').append(`
          ...
          <a href="#" onclick="stop_scan('/scan/${project}/${scan_object.id}/stop', scan_id=${scan_object.id}, subscan_id=null, reload_scan_bar=true, reload_location=false)" class="btn btn-xs btn-soft-danger waves-effect waves-light mt-1 float-end"><i class="fe-alert-triangle"></i> Stop</a>
          </div>
          </a>
          </div>
          `);
