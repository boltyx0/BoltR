	...
	var interesting_subdomain_table = $('#interesting_subdomains').DataTable({
		"drawCallback": function(settings, start, end, max, total, pre) {
			...
			if (this.fnSettings().fnRecordsTotal() == 0) {
				...
				// <div class="card-body">
				// BoltR could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
				// </div>
				// </div>`);
			} else {
				// show nav bar
...
function stop_scan(url, scan_id=null, subscan_id=null, reload_scan_bar=true, reload_location=false) {
	...
	// #region agent log
	try {
		fetch('http://127.0.0.1:7242/ingest/88dfb394-5668-4c40-86ed-91f5ae5611c9', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				id: 'log_' + Date.now() + '_stop_scan_called',
				timestamp: Date.now(),
				location: 'web/static/custom/custom.js:stop_scan:before',
				message: 'stop_scan invoked',
				runId: 'pre-fix',
				hypothesisId: 'H1',
				data: { url: url, scan_id: scan_id, subscan_id: subscan_id }
			})
		}).catch(function(){});
	} catch(e) {}
	// #endregion agent log
				...
				return response.json().then(function(respData) {
					...
					try {
						fetch('http://127.0.0.1:7242/ingest/88dfb394-5668-4c40-86ed-91f5ae5611c9', {
							...
							headers: { 'Content-Type': 'application/json' },
							body: JSON.stringify({
								id: 'log_' + Date.now() + '_stop_scan_response',
								timestamp: Date.now(),
								location: 'web/static/custom/custom.js:stop_scan:after',
								message: 'stop_scan response',
								runId: 'pre-fix',
								hypothesisId: 'H1',
								data: { httpStatus: response.status, body: respData }
							})
						}).catch(function(){});
					} catch(e) {}
					// #endregion agent log
		...
		if (response.status) {
			...
			fetch(`${endpoint_url}?format=json&ip_domain=${domain_name}`, {
				...
				if (response.status) {
					...
					Swal.fire({
						title: 'Oops!',
						text: `BoltR could not fetch WHOIS records for ${domain_name}!`,
						icon: 'error'
					});
				}
			});
...
function get_domain_whois(whoisLookupUrl, domain_name, addTargetUrl, project_slug, show_add_target_btn=false) {
	...
	fetch(url, {
		...
		if (response.status) {
			...
			Swal.fire({
				title: 'Oops!',
				text: `BoltR could not fetch WHOIS records for ${domain_name}! ${response['message']}`,
				icon: 'error'
			});
		}
	});
	...
	fetch(endpoint_url, {
		...
		if (response.status) {
			...
			$('#most_vulnerable_target_div').append(`
				...
				Once the vulnerability scan is performed, BoltR will identify the most vulnerable targets.</div>
			`);
		}
	});
}
	...
	fetch(endpoint_url + '?format=json', {
		...
		if (response.status) {
			...
			$('#most_common_vuln_div').append(`
				...
				Once the vulnerability scan is performed, BoltR will identify the Most Common Vulnerabilities.</div>
			`);
		}
	});
}
...
/* BoltR: Keep second nav bar visible if theme adds "active" to ul.navbar-nav (e.g. when using standalone top_nav) */
$(function () {
	$('.topnav .navbar-nav').removeClass('active');
	$('.topnav #topnav-menu-content').addClass('show').css('display', 'flex');
});
